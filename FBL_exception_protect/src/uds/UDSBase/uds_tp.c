/******************************************************************************
* 文件名称: uds_tp.c
* 内容摘要: UDS 协议栈 TP 层，基于 ISO 15765-2
* 创建者の: 孔佳伟
...
// 接收连续帧计数标志，当其 >= NT_XMIT_FC_BS (前提条件：NT_XMIT_FC_BS 不为 0) 时，应返回一帧流控帧，并清零复位
...
static int recv_singleframe (uint8_t* frame_buf, uint8_t frame_dlc)
{
    ...
    #else
    {
        ...
        // 有效数据长度合法性检测
        if (uds_dlc > FRAME_SIZE - 2 || 0 == uds_dlc)
            return -1;
...
static int send_singleframe (uint8_t* msg_buf, uint16_t msg_dlc)
{
    ...
    #if(FRAME_SIZE <= 8)
    {
        if (0 == msg_dlc || msg_dlc > (FRAME_SIZE - 1)) 
            return -1;
    ...
    #else
    {
        if (0 == msg_dlc || msg_dlc > (FRAME_SIZE - 2)) 
            return -1;
